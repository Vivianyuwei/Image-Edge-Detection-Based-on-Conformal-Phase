...
M0=-1/(size1^2)*ones(size1);  M0(floor(size1/2)+1,floor(size1/2)+1)=(size1^2-1)/size1^2;  image0=conv2(double(image),double(M0),'same');
for k=1:3
...
%     pcc(floor(size1/2)+1,floor(size1/2)+1)=-1*pcc(floor(size1/2)+1,floor(size1/2)+1);
    ...
    rp=conv2(double(image0),double(pcc),'same'); rx=conv2(double(image),double(uc),'same'); ry=conv2(double(image),double(vc),'same');  rz=conv2(double(image0),double(wc),'same');
...
M1= phasecong3(image,nscale,4,3,2.0,0.55,k4,cutoff,10,noiseMethod);
