...
function [CP]=ClassifyOnNN2(DM,trainClassIDs,testClassIDs,class_Num)
% Version 1.0
...
for i=1:length(testClassIDs);
    [distNew, index]= max(DM(i,:));   % find Nearest Neighborhood
    if index == testClassIDs(i)  % judge whether the nearest one is correctly classified
