...
function [CLBP_S,CLBP_M,CLBP_C,CLBP_T] = cmbp(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.2
...
[r,c]=size(d_C);
DiffThreshold=zeros(r,c);
for i=1:neighbors
    DiffThreshold=DiffThreshold+Diff{i};
end
DiffThreshold=DiffThreshold/neighbors;
Image_Threshold=mean(MeanDiff);
...
% CLBP_C
Mean_C{neighbors+1}=d_C;
Image_C=zeros(r,c);
for i=1:neighbors+1
    Image_C=Image_C+Mean_C{i};
end
Image_C=Image_C/(neighbors+1);
CLBP_C = Image_C>=mean(d_image(:));
% CLBP_C = d_C>=mean(d_image(:));
CLBP_T=DiffThreshold>=Image_Threshold;
